# Feature covers all admin login scenarios including first-time login and re-login after logout
@admin @authentication @smoke
Feature: Admin Dashboard Login
  As an admin user
  I want to log in to the Qawafel admin dashboard
  So that I can manage the platform

  # Verifies that a known admin user can log in successfully with valid credentials
  @login @happy-path @regression
  ...
  # Verifies that a different admin user can log in after the previous session is logged out
  @login @logout @regression
  Scenario: Successful login with new credentials
